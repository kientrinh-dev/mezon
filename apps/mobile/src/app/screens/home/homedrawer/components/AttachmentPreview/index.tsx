    ...
    return (
        <View style={styles.container} {...testProperties('attachmentPreview.container', true)}>
			<LinearGradient
                ...
                {...testProperties('attachmentPreview.gradient', true)}
            />
            <ScrollView
                horizontal
            ...
            >
				{attachmentFilteredByChannelId.files.map((attachment, index) => {
					...
					return (
                        <View key={index + attachment.filename} style={styles.attachmentItem} {...testProperties(`attachmentPreview.item.${index}`)}>
							{isFile ? (
